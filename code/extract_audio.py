...
OUTPUT_NPZ = Path("mvt_audio_dataset_8fps.npz")
...
def extract(wav):
    ...
    mel = mean_bins(log_mel, t, n_bins)
...
def main():
    wavs = sorted(PROCESSED_DIR.glob("*_audio.wav"))
    Xs, Ts, names = [], [], []

    for wp in wavs:
        stem = wp.stem.replace("_audio", "")
        X, t = extract(wp)
        Xs.append(X); Ts.append(t); names.append(stem)
